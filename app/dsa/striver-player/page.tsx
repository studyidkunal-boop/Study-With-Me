...
function StriverPlayerContent() {
  ...
  // Sync video parameter on search query
  useEffect(() => {
    if (videoParam && orderedVideos.length > 0) {
      const found = orderedVideos.find((v) => v.id === videoParam);
      if (found) {
        setCurrent(found);
      }
    } else if (orderedVideos.length > 0 && !current) {
      setCurrent(orderedVideos[0]);
    }
  }, [videoParam, orderedVideos, current]);

  // Load note and track last-watched lecture whenever the selected video changes
  useEffect(() => {
    if (current) {
  ...
  const filteredVideos = useMemo(() => {
    return orderedVideos.filter((v) => {
      ...
      if (filter === "bookmarked") {
        return matchesSearch && isVideoBookmarked(v.id);
      }
  ...
  return (
    <div className="flex-grow flex flex-col md:flex-row relative bg-transparent text-white">
      ...
      <aside 
        className={`
          fixed inset-y-0 left-0 z-35 w-80 md:w-96 glass-panel border-r border-zinc-800/60 flex flex-col transform transition-transform duration-300 md:relative md:translate-x-0
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    className={`mt-0.5 p-1 rounded-lg text-sm transition-colors opacity-65 hover:opacity-100 hover:bg-zinc-850 shrink-0 cursor-pointer
      ...
      {current ? (
        <main className="flex-grow p-4 md:p-8 flex flex-col gap-6 md:gap-8 overflow-y-auto max-w-7xl mx-auto w-full">
          
          {/* PLAYER HEADER (Requirement 10) */}
          <section className="p-4 rounded-xl border border-zinc-800 bg-zinc-950/60 backdrop-blur flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            ...
            <button
              onClick={handleSwitchMentor}
              className="text-[10px] font-extrabold uppercase px-3 py-1.5 rounded-lg border border-zinc-700 bg-zinc-900 hover:bg-zinc-850 text-zinc-300 hover:text-white cursor-pointer transition"
            >
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-800/50 pb-5">
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <button
                ...
                className={`flex items-center gap-1 text-xs px-4 py-2 rounded-xl border transition-all duration-200 font-bold cursor-pointer
                  ${
                    isVideoBookmarked(current.id)
          ...
          <section className="glass-panel border border-zinc-800/80 rounded-2xl p-5 md:p-6 shadow-xl flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                {isSavedIndicator && (
                  <span className="text-[11px] text-emerald-400 font-bold bg-emerald-950/40 px-2 py-0.5 rounded border border-emerald-500/20">
                    Saved
                  </span>
                )}
                <button
                  onClick={exportNotes}
                  className="text-xs font-semibold px-3 py-1.5 bg-zinc-900 border border-zinc-800 rounded-lg text-zinc-400 hover:text-zinc-200 hover:bg-zinc-850 transition cursor-pointer"
                >
                  📥 Export All Notes
                </button>
              </div>
        ...
        <div className="flex-grow flex flex-col items-center justify-center p-8 text-zinc-500">
      ...
      <Suspense fallback={
        <div className="flex-grow flex items-center justify-center bg-zinc-950">
          <div className="animate-pulse flex flex-col items-center gap-3">
            ...
            <span className="text-sm font-semibold text-zinc-500">Preparing player stream...</span>
          </div>
        </div>
      }>
